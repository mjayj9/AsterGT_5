...
func run() -> void:
 ...
 await frames(40)
 check("main_menu_has_six_actions",game.hud.page=="home" and game.hud.menu_root.find_children("*","Button",true,false).size()==6)
 ...
 game.start_drive("Free Drive");game.car.test_input={"brake":1.0};await frames(120)
 ...
 check("render_scale_applied",absf(game.get_viewport().scaling_3d_scale-loads[3].scale)<.01)
 game.apply_preset(2)
