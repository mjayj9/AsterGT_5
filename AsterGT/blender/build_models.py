...
for z,w,t in sections:
 for x,y in [(0,t),(.68*w,t-.008),(.91*w,t-.03),(w,t-.10),(w*.99,-.13),(.90*w,-.28),(0,-.29),(-.90*w,-.28),(-.99*w,-.13),(-w,t-.10),(-.91*w,t-.03),(-.68*w,t-.008)]:
  arch_bump=max(math.exp(-((z+1.39)/.55)**2),math.exp(-((z-1.39)/.55)**2))
  shoulder_lift=.14*arch_bump*min(1,max(0,(abs(x)/w-.5)/.3)) if y>-.12 else 0
  verts.append((x,y+shoulder_lift,z))
faces=[];n=12
...
for z in [-1.39,1.39]:
 cut=cyl('Arch cutter',(0,-.115,z),.417,2.8,None,vertices=64)
...
for side in [-1,1]:
 ...
 for z in [-1.64,-1.52,-1.40]:cube('Fender_vent_'+str(s),(s*.68,.355,z),(.20,.017,.045),carbon,.015)
